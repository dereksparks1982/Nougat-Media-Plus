...
# Nougat Media Suite v0.0.29 changed-files installer.
# Approved scope: TV Up Next/autoplay/Back-to-Series reliability plus the owner-approved
# carry-forward Home wheel, Vimeo, Video Player surround, and Home artwork repairs.
...
manifest_path="$payload_root/NOUGAT_MEDIA_SUITE_PATCH_MANIFEST_v29.json"
archive_parent="$HOME/DKLab/Archives/ReddMedia Archive"
stamp="$(date +%Y%m%d_%H%M%S)"
rollback_root="$archive_parent/Nougat_Media_Suite_pre_v0_0_29_TV_RELIABILITY_UI_REPAIR_$stamp"
build_root="${TMPDIR:-/tmp}/nougat-media-suite-v0_0_29-$stamp-$$"
expected_head="80b12f2b47959f4482e65238b9c0b6033f9785a2"
...
modified_paths=(
  ...
  "src/main.cpp"
  "tools/test_nougat_media_suite_retained_v28.py"
  "tools/test_nougat_media_suite_v29.py"
  "tools/test_nougat_media_suite_ui_smoke_v29.py"
  "tools/test_installer_rollback_v29.py"
  ...
  "INSTALL_NOUGAT_MEDIA_SUITE_v0_0_29.sh"
  "NOUGAT_MEDIA_SUITE_PATCH_MANIFEST_v29.json"
)
...
s.close()
if used:
    print('FAIL: localhost port 8096 is in use. Close Nougat Media Suite/Jellyfin before applying v0.0.29.')
    raise SystemExit(1)
...
verify_git_state() {
  ...
  [[ "$head" == "$expected_head" ]] || { printf 'FAIL: expected accepted v0.0.28 HEAD %s, found %s\n' "$expected_head" "${head:-unknown}"; return 1; }
  [[ -z "$dirty" ]] || { printf 'FAIL: accepted v0.0.28 worktree must be clean before v0.0.29:\n%s\n' "$dirty"; return 1; }
  printf 'Git preflight PASS: clean main at accepted v0.0.28 commit %s.\n' "$expected_head"
}
...
if m.get('target_version') != '0.0.29': print('FAIL: manifest target version mismatch'); raise SystemExit(1)
if m.get('accepted_base_commit') != '80b12f2b47959f4482e65238b9c0b6033f9785a2': print('FAIL: manifest accepted base mismatch'); raise SystemExit(1)
def sha(p):
    h=hashlib.sha256()
    with p.open('rb') as f:
        for b in iter(lambda:f.read(1024*1024),b''): h.update(b)
...
for rel,record in m.get('base_files',{}).items():
    p=project/rel
    if not p.is_file() or p.stat().st_size!=record['bytes'] or sha(p)!=record['sha256']:
        print(f'FAIL: accepted v0.0.28 base differs: {rel}'); raise SystemExit(1)
for rel in m.get('required_absent',[]):
    if (project/rel).exists(): print(f'FAIL: expected accepted-v0.0.28-absent path exists: {rel}'); raise SystemExit(1)
print(f"Manifest verified: {len(m.get('payload',{}))} payload files, {len(m.get('base_files',{}))} exact accepted-v0.0.28 base files, {len(m.get('required_absent',[]))} required-absent paths.")
PY
}
...
print('Licensing, Search engine/bridge, and existing P2P implementation preserved unchanged for v0.0.29; focused P2P expansion remains v0.0.30.')
PY
}
...
save_rollback_snapshot() {
  ...
  if [[ -e "$project_root/Nougat_Media_Suite_v28" ]]; then cp -a "$project_root/Nougat_Media_Suite_v28" "$rollback_root/project/Nougat_Media_Suite_v28" || return 1; fi
  for src in "$launcher_unversioned" "$launcher_v22" "$launcher_v23" "$launcher_v24" "$launcher_v25" "$launcher_v26" "$launcher_v27" "$launcher_v28" "$launcher_v29" "$canonical_launcher"; do
...
Created: $(date -Is)
Accepted base: v0.0.28 $expected_head
Target candidate: v0.0.29
Package scope: TV reliability and carry-forward UI/artwork repair; P2P expansion v0.0.30 deferred
INFO
}
...
apply_payload() {
  local rel
  for rel in "${modified_paths[@]}"; do
    [[ "$rel" == "NOUGAT_MEDIA_SUITE_PATCH_MANIFEST_v29.json" ]] && continue
    mkdir -p "$project_root/$(dirname "$rel")" || return 1
    cp -a "$payload_root/$rel" "$project_root/$rel" || { printf 'FAIL: could not apply %s
' "$rel"; return 1; }
  done
  cp -a "$payload_root/NOUGAT_MEDIA_SUITE_PATCH_MANIFEST_v29.json" "$project_root/NOUGAT_MEDIA_SUITE_PATCH_MANIFEST_v29.json" || return 1
  chmod +x "$project_root/INSTALL_NOUGAT_MEDIA_SUITE_v0_0_29.sh" \
    "$project_root/tools/test_nougat_media_suite_retained_v28.py" \
    "$project_root/tools/test_nougat_media_suite_v29.py" \
    "$project_root/tools/test_nougat_media_suite_ui_smoke_v29.py" \
    "$project_root/tools/test_installer_rollback_v29.py" || return 1
  applied=1
}
...
restore_rollback() {
  [[ "$applied" == "1" ]] || return 0
  printf '
ROLLBACK START: restoring exact accepted v0.0.28 touched state
'
  ...
  rm -f -- "$project_root/Nougat_Media_Suite_v29"
  if [[ -e "$rollback_root/project/Nougat_Media_Suite_v28" ]]; then cp -a "$rollback_root/project/Nougat_Media_Suite_v28" "$project_root/Nougat_Media_Suite_v28"; chmod +x "$project_root/Nougat_Media_Suite_v28" 2>/dev/null || true; fi
  for dest in "$launcher_unversioned" "$launcher_v22" "$launcher_v23" "$launcher_v24" "$launcher_v25" "$launcher_v26" "$launcher_v27" "$launcher_v28" "$launcher_v29" "$canonical_launcher"; do
  ...
  printf 'ROLLBACK PASS: accepted v0.0.28 touched state restored.
'
}
...
run_source_tests() {
  ...
  python3 "$project_root/tools/test_nougat_media_suite_retained_v28.py" "$project_root" || return 1
  python3 "$project_root/tools/test_nougat_media_suite_v29.py" "$project_root" || return 1
  python3 "$project_root/tools/test_installer_rollback_v29.py" "$project_root" || return 1
}
...
build_stub() {
  ...
  local exe="$build_root/stub/Nougat_Media_Suite_v29"
  [[ "$("$exe" --version)" == "Nougat Media Suite v0.0.29" ]] || { printf 'FAIL: v29 stub version mismatch
'; return 1; }
  ...
  python3 "$project_root/tools/test_nougat_media_suite_retained_v28.py" "$project_root" "$exe" || return 1
  python3 "$project_root/tools/test_nougat_media_suite_v29.py" "$project_root" "$exe" || return 1
  python3 "$project_root/tools/test_nougat_media_suite_ui_smoke_v29.py" "$project_root" "$exe" || return 1
}
...
build_full() {
  ...
  local exe="$build_root/full/Nougat_Media_Suite_v29"
  verify_relative_ai_rpath "$exe" || return 1
  [[ "$(env -u LD_LIBRARY_PATH "$exe" --version 2>/dev/null)" == "Nougat Media Suite v0.0.29" ]] || { printf 'FAIL: full native v29 version/runtime check failed
'; return 1; }
...
legacy={'NougatMediaSuite.desktop','NougatMediaSuite_v22.desktop','NougatMediaSuite_v23.desktop','NougatMediaSuite_v24.desktop','NougatMediaSuite_v25.desktop','NougatMediaSuite_v26.desktop','NougatMediaSuite_v27.desktop','NougatMediaSuite_v28.desktop','NougatMediaSuite_v29.desktop',canonical}
try:
...
apply_raw_icon() {
  ...
  [[ "$info" == *"file://$master_icon"* ]] || { printf 'FAIL: v29 executable custom-icon readback mismatch\n%s\n' "$info"; return 1; }
  printf 'Raw executable approved N custom-icon metadata verified after final write.\n'
}

install_final_candidate() {
  cp "$build_root/full/Nougat_Media_Suite_v29" "$project_root/Nougat_Media_Suite_v29" || return 1
  chmod +x "$project_root/Nougat_Media_Suite_v29" || return 1
  verify_relative_ai_rpath "$project_root/Nougat_Media_Suite_v29" || return 1
  [[ "$(env -u LD_LIBRARY_PATH "$project_root/Nougat_Media_Suite_v29" --version 2>/dev/null)" == "Nougat Media Suite v0.0.29" ]] || return 1
  env -u LD_LIBRARY_PATH "$project_root/Nougat_Media_Suite_v29" --discover-ai-self-test || return 1
  env -u LD_LIBRARY_PATH "$project_root/Nougat_Media_Suite_v29" --v25-ui-state-self-test || return 1
  env -u LD_LIBRARY_PATH "$project_root/Nougat_Media_Suite_v29" --v28-ui-state-self-test || return 1
  env -u LD_LIBRARY_PATH "$project_root/Nougat_Media_Suite_v29" --v29-tv-reliability-self-test || return 1
  python3 "$project_root/tools/test_nougat_media_suite_retained_v28.py" "$project_root" "$project_root/Nougat_Media_Suite_v29" || return 1
  python3 "$project_root/tools/test_nougat_media_suite_v29.py" "$project_root" "$project_root/Nougat_Media_Suite_v29" || return 1
  python3 "$project_root/tools/test_nougat_media_suite_ui_smoke_v29.py" "$project_root" "$project_root/Nougat_Media_Suite_v29" || return 1
  install_icon_aliases || return 1
  install_launchers || return 1
  refresh_nougat_favorite || return 1
  apply_raw_icon "$project_root/Nougat_Media_Suite_v29" || return 1
  rm -f -- "$project_root/Nougat_Media_Suite_v28" || return 1
  command -v nautilus >/dev/null 2>&1 && nautilus -q >/dev/null 2>&1 || true
  return 0
}

main() {
  phase_start "accepted v0.0.28 base and v0.0.29 prerequisites"
  [[ -d "$project_root" ]] || { printf 'FAIL: project not found: %s
' "$project_root"; printf '
FINAL FAIL: v0.0.29 not installed. Terminal remains open.
'; return 1; }
  local cmd
...
'; return 1; }
  [[ -x "$project_root/Nougat_Media_Suite_v28" ]] || { printf 'FAIL: accepted v0.0.28 root executable missing/not executable.
'; return 1; }
  [[ "$(env -u LD_LIBRARY_PATH "$project_root/Nougat_Media_Suite_v28" --version 2>/dev/null)" == "Nougat Media Suite v0.0.28" ]] || { printf 'FAIL: accepted v0.0.28 executable version/runtime check failed.
'; return 1; }
...
FINAL FAIL: protected accepted-v0.0.28 state mismatch. Terminal remains open.
'; return 1; }
  verify_manifest || { printf '
FINAL FAIL: v0.0.29 package/base verification failed. Terminal remains open.
'; return 1; }
  phase_pass "accepted v0.0.28 base and v0.0.29 prerequisites"

  phase_start "save exact accepted v0.0.28 rollback snapshot"
  save_rollback_snapshot || { printf '
FINAL FAIL: rollback snapshot failed before changes. Terminal remains open.
'; return 1; }
  phase_pass "save exact accepted v0.0.28 rollback snapshot"

  phase_start "apply approved Nougat Media Suite v0.0.29 changed files"
  apply_payload || { restore_rollback; printf '
FINAL FAIL: v0.0.29 payload application failed. Terminal remains open.
'; return 1; }
  phase_pass "apply approved Nougat Media Suite v0.0.29 changed files"

  phase_start "license, Search/P2P preservation, retained v0.0.28 behavior, and v0.0.29 regression tests"
  run_source_tests || { restore_rollback; printf '
FINAL FAIL: source/regression validation failed; accepted v0.0.28 touched state restored. Terminal remains open.
'; return 1; }
  phase_pass "license, Search/P2P preservation, retained v0.0.28 behavior, and v0.0.29 regression tests"

  phase_start "warnings-as-errors stub build and X11 v0.0.29 identity/UI smoke"
  mkdir -p "$build_root" && build_stub || { restore_rollback; printf '
FINAL FAIL: deterministic build/UI validation failed; accepted v0.0.28 touched state restored. Terminal remains open.
'; return 1; }
  phase_pass "warnings-as-errors stub build and X11 v0.0.29 identity/UI smoke"

  phase_start "full native Nougat Media Suite v0.0.29 rebuild"
  build_full || { restore_rollback; printf '
FINAL FAIL: full native v0.0.29 build failed; accepted v0.0.28 touched state restored. Terminal remains open.
'; return 1; }
  phase_pass "full native Nougat Media Suite v0.0.29 rebuild"

  phase_start "install and verify final v29 executable, launchers, and N identity"
  install_final_candidate || { restore_rollback; printf '
FINAL FAIL: final v0.0.29 installation failed; accepted v0.0.28 touched state restored. Terminal remains open.
'; return 1; }
  phase_pass "install and verify final v29 executable, launchers, and N identity"

  rm -rf -- "$build_root"
  printf '
FINAL PASS: Nougat Media Suite v0.0.29 TV Playback, Navigation, and Carry-Forward UI Repair installed and validated.
'
  printf 'Executable: %s
' "$project_root/Nougat_Media_Suite_v29"
  printf 'Rollback snapshot: %s
' "$rollback_root"
  printf 'LICENSE STATUS: protected PolyForm Noncommercial licensing files preserved unchanged.
'
  printf 'SEARCH/P2P STATUS: Search engine/bridge and existing P2P implementation preserved unchanged; focused P2P expansion remains roadmap v0.0.30.
'
  printf 'OWNER CHECK REQUIRED: same-folder next-episode discovery from Home/Library/Open File/resume; immediate Up Next with visible 10-second countdown; automatic next-episode playback; manual Stop cancellation; Back to Series returning to the actual show/season; Home top-tab wheel routing; Vimeo directly after YouTube with Vimeo branding; uniform Video Player background with no partial brown rail; TV episode/season/series artwork fallback; movie artwork filling Home card regions; retained v0.0.28 behavior.
'
  printf 'CANDIDATE STATUS: v0.0.29 remains unaccepted until owner real-machine visual/use approval.
'
  printf 'Launch: cd "%s" && ./Nougat_Media_Suite_v29
' "$project_root"
}
