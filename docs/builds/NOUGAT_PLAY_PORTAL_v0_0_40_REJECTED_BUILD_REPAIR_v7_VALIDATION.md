# Nougat Play Portal v0.0.40 Accepted Validation
...
Revision v7 fixes the confirmed v6 executable-staging defect: an existing rejected root `Nougat_Play_Portal_v40` could be symlinked into the temporary candidate tree, then copied back as a dangling/self-referencing symlink. v7 excludes versioned executables from candidate mirroring, copies the linked binary with symlink dereferencing, and requires regular-file/ELF gates both before and after promotion to the project root.
