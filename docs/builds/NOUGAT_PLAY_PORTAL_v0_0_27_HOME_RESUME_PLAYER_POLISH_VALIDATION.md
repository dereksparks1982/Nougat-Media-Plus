# Nougat Play Portal v0.0.27 Validation Record
...
- Stub executable `--version`: PASS (`Nougat Play Portal v0.0.27`).
...
- Root cause: the v27 installer incorrectly invoked the historical `tools/test_nougat_play_portal_v26.py` identity contract after applying v27 sources. That contract intentionally requires v0.0.26 CMake/executable identity and therefore cannot be a valid post-upgrade compatibility test.
- Repair: removed that obsolete invocation; retained `tools/test_nougat_play_portal_retained_v26.py` for accepted-v26 behavioral compatibility under v27 identity.
- Added a static installer regression assertion that fails if `tools/test_nougat_play_portal_v26.py` is ever reintroduced into `run_source_tests()` for v27.
...
- final root `Nougat_Play_Portal_v27` install, executable/version/RPATH verification;
