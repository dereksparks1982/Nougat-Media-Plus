# Nougat Play Portal v0.0.35 Validation
...
- root executable installation and version check for `Nougat_Play_Portal_v35`;
...
- The replacement repair installer recognizes the exact interrupted state left by the earlier rollback bug, where the first v0.0.35 candidate source was restored but `NOUGAT_PLAY_PORTAL_PATCH_MANIFEST_v35.json` and the root `Nougat_Play_Portal_v35` executable were incorrectly removed.
