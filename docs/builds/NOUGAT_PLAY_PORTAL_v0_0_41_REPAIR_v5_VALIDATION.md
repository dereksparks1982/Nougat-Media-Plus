# Nougat Play Portal v0.0.41 rejected-build repair v5 validation
...
- `tools/test_nougat_play_portal_v41.py` source/behavior contract.
...
- Final root contains exactly one `Nougat_Play_Portal_v*` executable: `Nougat_Play_Portal_v41`.
- No versioned `NougatPlayPortal_v*.desktop` files may remain in the project root or user launcher directory.
- Both canonical launchers must point to `Nougat_Play_Portal_v41`.
