# Nougat Play Portal v0.0.63 - Games & Emulator Overhaul Build 3
...
Build 2 compiled `Nougat_Play_Portal_v63` successfully with warnings-as-errors, but its post-build version test executed the binary from the temporary CMake build directory. Because Nougat's AI runtime uses `$ORIGIN/components/ai/runtime`, that temporary location could not resolve `libllama.so.0`.
...
- `Nougat_Play_Portal_v62` is not removed until all v63 runtime validation gates pass.
