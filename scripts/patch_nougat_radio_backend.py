...
def main():
    if len(sys.argv) != 2:
        die("usage: patch_nougat_radio_backend.py <project-root>")
    ...
    for p in (hpp_path, cpp_path, cmake_path):
        if not p.exists():
            die(f"missing project source: {p}")
    ...
    if "V4l2RadioProvider v4l2_radio_;" not in hpp:
        hpp = replace_once(
            ...
            "    V4l2RadioProvider v4l2_radio_; // NOUGAT_V67_HVR955Q_FM\n"
            "    pid_t receive_pid_ = -1;",
            "V4L2 provider member",
        )
...
'''
    if "find_cx231xx_alsa_capture()" not in cpp:
        cpp = replace_once(cpp, helper_anchor, helper_code, "ALSA capture helper")

    radio_block_pattern = re.compile(
    ...
    if "Native HVR/V4L2 radio receiver" not in cpp:
        m = radio_block_pattern.search(cpp)
        if not m:
            die("V4L2 radio discovery block not found")
        new_block = r'''    for (const std::string& path : glob_paths("/dev/radio*")) {
        ...
        device.notes = "Native HVR/V4L2 radio receiver. Tuning is performed by the kernel tuner driver, not rtl_fm.";
        device.receive = true;
    ...
    for (const std::string& path : glob_paths("/dev/dvb/adapter*/frontend*"))'''
        cpp = cpp[:m.start()] + new_block + cpp[m.end():]
    ...
    if "v4l2_radio_.close_device();" not in cpp:
        cpp = replace_once(
            ...
            "    state_.receiving = false;",
            "V4L2 provider release",
        )

    spawn_anchor = '''bool RadioBackend::spawn_receive_pipeline_locked(std::string& status) {
    if (!state_.rtl_available) {'''
    if "NOUGAT_V67_HVR955Q_NATIVE_RECEIVE" not in cpp:
        spawn_new = r'''bool RadioBackend::spawn_receive_pipeline_locked(std::string& status) {
    if (state_.selected_device >= 0 &&
        ...
        // NOUGAT_V67_HVR955Q_NATIVE_RECEIVE
        const RadioDevice& device = state_.devices[static_cast<std::size_t>(state_.selected_device)];

        if (state_.modulation != RadioModulation::WFM) {
            status = "The WinTV-HVR-955Q native radio path supports broadcast FM/WFM only.";
            state_.status = status;
            return false;
        }
        if (!executable_available("arecord") || !executable_available("aplay")) {
            status = "ALSA arecord/aplay are required for the HVR-955Q FM audio path.";
            state_.status = status;
            return false;
        }
        ...
        if (capture.empty()) {
            v4l2_radio_.close_device();
            status = "The HVR-955Q radio tuned, but the cx231xx ALSA capture device was not found.";
            state_.status = status;
            return false;
        }
        ...
        if (state_.recording) {
            ...
            current_recording_path_ = recordings_dir() + "/fm-" + timestamp_name() + ".wav";
            command << " | tee >(aplay -q -t raw -f S16_LE -r 48000 -c 2)"
                    << " | ffmpeg -nostdin -loglevel error -y -f s16le -ar 48000 -ac 2 -i pipe:0 "
        ...
        if (pid < 0) {
            v4l2_radio_.close_device();
            status = std::string("Could not start HVR FM audio capture: ") + std::strerror(errno);
            state_.status = status;
            return false;
        }
        if (pid == 0) {
        ...
        text << "HVR-955Q FM receiving " << std::fixed << std::setprecision(1)
    ...
    if (!state_.rtl_available) {'''
        cpp = replace_once(cpp, spawn_anchor, spawn_new, "native HVR receive branch")

    scan_anchor = '''    if (!executable_available("rtl_power")) {
    ...
    if "native_v4l2_scan" not in cpp:
        scan_new = r'''    bool native_v4l2_scan = false;
    {
        std::lock_guard<std::mutex> lock(mutex_);
        native_v4l2_scan =
    ...
    cancel_scan();'''
        cpp = replace_once(cpp, scan_anchor, scan_new, "native V4L2 scan selection")
    ...
    if "NOUGAT_V67_HVR955Q_NATIVE_SCAN" not in cpp:
        finish_new = r'''void RadioBackend::finish_scan(double minimum_hz, double maximum_hz, double step_hz) {
    std::string native_path;
    {
        ...
        if (state_.selected_device >= 0 &&
            state_.selected_device < static_cast<int>(state_.devices.size()) &&
            state_.devices[static_cast<std::size_t>(state_.selected_device)].backend == "V4L2 Radio") {
            native_path = state_.devices[static_cast<std::size_t>(state_.selected_device)].id;
        }
    }

    if (!native_path.empty()) {
        // NOUGAT_V67_HVR955Q_NATIVE_SCAN
        ...
        const double bounded_min = std::max(minimum_hz, scanner.info().minimum_hz);
        const double bounded_max = scanner.info().maximum_hz > 0.0
            ? std::min(maximum_hz, scanner.info().maximum_hz)
            : maximum_hz;

        for (double hz = bounded_min; hz <= bounded_max && !scan_cancel_.load(); hz += step_hz) {
            if (!scanner.tune(hz, error)) continue;
        ...
        if (scan_cancel_.load()) {
            ...
            result << "Strongest FM activity near " << std::fixed << std::setprecision(1)
                   << (strongest_hz / 1000000.0) << " MHz (" << strongest_signal << "%).";
            state_.status = result.str();
        } else {
            state_.status = "FM scan completed, but the driver did not report a usable signal level.";
        }
        return;
    }

    const std::string output = cache_dir() + "/scan-" + timestamp_name() + ".csv";'''
        cpp = replace_once(cpp, finish_anchor, finish_new, "native V4L2 scan worker")
    ...
    print("PASS: HVR scan no longer depends on rtl_power")
    print("PASS: HVR listen path uses cx231xx ALSA capture instead of rtl_fm")
