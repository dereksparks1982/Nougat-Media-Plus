...
fetch_exact_ubuntu_source() {
    ...
    local source_ver="$2"
    local source_root="$3"
    local temp_sources="/tmp/nougat-v67-fix11-deb-src.sources"
    local installed_sources="/etc/apt/sources.list.d/nougat-v67-fix11-deb-src.sources"
...
main() {
    ...
    MATCHED="$BUILDROOT/fix11-matched-media-modules"
    BUILD_ALIAS="/tmp/nougat-v67-fix11-kbuild-$$"
    UPDATE_DIR="/lib/modules/$KREL/updates/nougat-v67-fm"
    APP_BUILD="$PROJECT/build/app-v67"
    KERNEL_PATCHER="$PROJECT/components/radio/kernel/hvr955q-fm/patch_kernel_source_fix11.py"
    APP_PATCHER="$PROJECT/scripts/patch_nougat_radio_backend_fix11.py"
    TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
    ARCHIVE="/home/dereksparks1982/DKLab/Archives/Nougat Play Portal/v67-fm-fix11-before-$TIMESTAMP"
    ...
    [[ -f "$KERNEL_PATCHER" && -f "$APP_PATCHER" ]] || { echo "FAIL: FIX11 patchers missing."; return 1; }
    ...
    echo "HVR-955Q FM RADIO END-TO-END DRIVER + APP + N DOCK ICON FIX11"
    echo "Kernel: $KREL"
    echo "============================================================"
    ...
    MATCHED_ALIAS="$BUILD_ALIAS/fix11-matched-media-modules"
...
EOF
    ...
    echo
    echo "=== BACK UP ONLY APP FILES FIX11 MAY CHANGE ==="
    ...
    local NEW_UPDATE="${UPDATE_DIR}.fix11-new-$$"
    local OLD_UPDATE="${UPDATE_DIR}.fix11-old-$$"
...
PY
    ...
    for mod in cx231xx cx231xx_dvb cx231xx_alsa si2157; do
        ...
        echo "$mod -> $path"
        case "$path" in "$UPDATE_DIR"/*) ;; *) echo "FAIL: $mod is not resolving to FIX11."; return 1 ;; esac
    done
    ...
    echo
    echo "============================================================"
    echo "PASS: FIX11 END-TO-END CANDIDATE INSTALLED"
