...
fetch_exact_ubuntu_source() {
    ...
    local source_ver="$2"
    local source_root="$3"
    local temp_sources="/tmp/nougat-v67-fix12-deb-src.sources"
    local installed_sources="/etc/apt/sources.list.d/nougat-v67-fix12-deb-src.sources"
...
main() {
    ...
    MATCHED="$BUILDROOT/fix12-matched-media-modules"
    BUILD_ALIAS="/tmp/nougat-v67-fix12-kbuild-$$"
    UPDATE_DIR="/lib/modules/$KREL/updates/nougat-v67-fm"
    APP_BUILD="$PROJECT/build/app-v67"
    KERNEL_PATCHER="$PROJECT/components/radio/kernel/hvr955q-fm/patch_kernel_source_fix12.py"
    APP_PATCHER="$PROJECT/scripts/patch_nougat_radio_backend_fix12.py"
    TIMESTAMP="$(date +%Y%m%d-%H%M%S)"
    ARCHIVE="/home/dereksparks1982/DKLab/Archives/Nougat Media Plus/v67-fm-fix12-before-$TIMESTAMP"
    ...
    [[ -f "$KERNEL_PATCHER" && -f "$APP_PATCHER" ]] || { echo "FAIL: FIX12 patchers missing."; return 1; }
    ...
    echo "HVR-955Q FM RADIO END-TO-END DRIVER + APP + N DOCK ICON FIX12"
    echo "Kernel: $KREL"
    echo "============================================================"
    ...
    echo "FIX12 verifies the build-dir app with Nougat's bundled llama runtime path."
    ...
    MATCHED_ALIAS="$BUILD_ALIAS/fix12-matched-media-modules"
...
EOF
    ...
    echo
    echo "=== BACK UP ONLY APP FILES FIX12 MAY CHANGE ==="
    ...
    echo
    echo "=== NORMALIZE CHANGED-SOURCE TIMESTAMPS ==="
    touch "$PROJECT/src/radio/radio_backend.cpp" \
          "$PROJECT/src/radio/radio_backend.hpp" \
          "$PROJECT/src/radio/v4l2_radio_provider.cpp" \
          "$PROJECT/src/radio/v4l2_radio_provider.hpp" \
          "$PROJECT/CMakeLists.txt" \
          "$PROJECT/src/main.cpp" \
          "$PROJECT/com.elderredsoftworks.NougatMediaPlus.desktop" || {
        restore_app_source "$ARCHIVE" "$PROJECT" "$new_cpp" "$new_hpp"
        echo "FAIL: could not normalize changed-source timestamps."
        return 1
    }
    ...
    local NMP_RUNTIME_LD
    NMP_RUNTIME_LD="$PROJECT/components/ai/runtime/lib:$PROJECT/components/ai/runtime/lib64"
    [[ -e "$PROJECT/components/ai/runtime/lib/libllama.so" || \
       -e "$PROJECT/components/ai/runtime/lib/libllama.so.0" || \
       -e "$PROJECT/components/ai/runtime/lib64/libllama.so" || \
       -e "$PROJECT/components/ai/runtime/lib64/libllama.so.0" ]] || {
        restore_app_source "$ARCHIVE" "$PROJECT" "$new_cpp" "$new_hpp"
        echo "FAIL: bundled llama runtime is missing from components/ai/runtime."
        return 1
    }

    VERSION="$(env LD_LIBRARY_PATH="$NMP_RUNTIME_LD${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" \
        "$BUILT" --version 2>&1)"
    [[ "$VERSION" == *"v0.0.67"* ]] || {
        restore_app_source "$ARCHIVE" "$PROJECT" "$new_cpp" "$new_hpp"
        echo "FAIL: wrong app identity: $VERSION"
        return 1
    }
    ...
    local NEW_UPDATE="${UPDATE_DIR}.fix12-new-$$"
    local OLD_UPDATE="${UPDATE_DIR}.fix12-old-$$"
...
PY
    ...
    for mod in cx231xx cx231xx_dvb cx231xx_alsa si2157; do
        ...
        echo "$mod -> $path"
        case "$path" in "$UPDATE_DIR"/*) ;; *) echo "FAIL: $mod is not resolving to FIX12."; return 1 ;; esac
    done
    ...
    echo
    echo "============================================================"
    echo "PASS: FIX12 END-TO-END CANDIDATE INSTALLED"
