...
fetch_exact_ubuntu_source() {
    ...
    local source_ver="$2"
    local source_root="$3"
    local temp_sources="/tmp/nougat-v67-fix10-deb-src.sources"
    local installed_sources="/etc/apt/sources.list.d/nougat-v67-fix10-deb-src.sources"
...
main() {
    ...
    MATCHED="$BUILDROOT/fix10-matched-media-modules"
    BUILD_ALIAS="/tmp/nougat-v67-fix10-kbuild-$$"
    ...
    [[ -f "$KERNEL_PATCHER" && -f "$APP_PATCHER" ]] || { echo "FAIL: FIX10 patchers missing."; return 1; }
    ...
    MATCHED_ALIAS="$BUILD_ALIAS/fix10-matched-media-modules"
...
EOF
    ...
    local NEW_UPDATE="${UPDATE_DIR}.fix10-new-$$"
    local OLD_UPDATE="${UPDATE_DIR}.fix10-old-$$"
...
PY
    ...
    for mod in cx231xx cx231xx_dvb cx231xx_alsa si2157; do
        ...
        echo "$mod -> $path"
        case "$path" in "$UPDATE_DIR"/*) ;; *) echo "FAIL: $mod is not resolving to FIX10."; return 1 ;; esac
    done
    ...
    echo
    echo "============================================================"
    echo "PASS: FIX10 END-TO-END CANDIDATE INSTALLED"
