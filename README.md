...
The Player provides file and network media playback with play and pause, stop, rewind and fast-forward, fullscreen, volume, progress tracking, chapter-aware seeking, clean television episode identity, resume support, episode progression, Up Next handling, keyboard and mouse controls, and Live TV channel navigation. Home provides Continue Watching and media shelves, while Library manages movies, television, music, videos, artwork, metadata, persistent media folders, search, list and grid views, playback history, and automatic media discovery. v0.0.57 adds persistent owner-controlled Fix Match metadata correction and stronger real-world filename normalization while preserving the existing Library structure.
...
Live TV supports over-the-air television hardware, tuner detection, scanning, channel storage, guide data, native playback, channel navigation, tuner diagnostics, real channel artwork, provider-neutral tuner handling, regional tuner models, physical-device grouping, and multiple independently allocatable tuner resources. Hardware support includes Hauppauge WinTV devices and HDHomeRun network tuners with automatic multi-device and multi-tuner enumeration so every available tuner can be managed independently.

World TV extends television beyond antenna reception with internet-accessible television sources, source verification, guide integration, asynchronous stream resolution, playback-health diagnostics, bounded connection handling, source diagnostics, and clear failure reporting. v0.0.57 expands World TV recovery through official broadcaster-page discovery, direct stream candidates, and bounded alternate-source retries.

Radio provides a unified receiver environment for AM, FM, shortwave, weather radio, DAB and DAB+, DRM, internet radio, SDR, favorites, recordings, direct frequency entry, band plans, modulation selection, bandwidth and filtering, gain and AGC controls, squelch, scanning, signal monitoring, spectrum and waterfall foundations, presets, and hardware-aware capability reporting. The receiver architecture supports automatic hardware discovery, multiple devices, multiple tuner resources, provider-neutral and region-aware receiver models, expandable receiver backends, and truthful capability reporting when hardware is unavailable, unsupported, experimental, or untested. v0.0.52 adds the native Nougat Radio backend and receiver architecture, experimental Hauppauge WinTV-HVR-955Q and Silicon Labs Si2157 FM support work, expanded SDR foundations, and integrated source foundations for SoapySDR, liquid-dsp, GNU Radio 4, KissFFT, and OP25 so future supported receiver hardware can plug into the same Nougat radio system. v0.0.57 consolidates Radio into one professional receiver interface with Local, Internet, Emergency, Weather, Shortwave, and ISS/Satellite sources presented together.

Nougat Radio is expanding into a universal receiver and scanner environment covering **Local, Internet, AM, FM, HD Radio, DAB/DAB+, DRM, Longwave, Mediumwave, Shortwave, Weather, Emergency, Public Safety, Police, Fire, EMS, Government, Military, Airband, Marine, Railroad, CB, FRS/GMRS, MURS, Amateur/Ham, Business, Utilities, Trunked Radio, P25, DMR, NXDN, TETRA, Paging, Numbers Stations, Time/Beacon, ADS-B, ACARS, AIS, Weather Satellite, Satellite, Amateur Satellite, and ISS/Satellite**. The service surface intentionally remains broad instead of hiding modes simply because the currently connected hardware cannot receive them. Nougat can report the receiver, tuner, SDR, decoder, or backend required for unavailable services. Digital and data-oriented services can use decoded-data views instead of pretending every signal is ordinary audio, while encrypted systems remain identified as encrypted or locked rather than being decrypted. The receiver program also includes scanning, direct frequency entry, modulation selection, tuning steps, bandwidth and filtering, gain and AGC, squelch, signal monitoring, favorites, recordings, presets, spectrum and waterfall foundations, automatic hardware discovery, multiple receiver resources, provider-neutral hardware handling, and expandable SDR and decoder backends.

Nougat's **Cellular Lab** extends the receiver architecture into an isolated private cellular-network laboratory. The program includes a private **2G GSM** network target with SDR/BTS hardware abstraction, BTS/BSC/MSC/HLR integration and status, network identity and channel allocation, attached enrolled test devices, subscriber and SIM management, local calls and SMS, signal and RF monitoring, protocol and diagnostic information, alarms, network-health reporting, spectrum tools, and later isolated GPRS packet-data support. A separate **1G Analog** laboratory is roadmapped as a later historical SDR/emulation project. Cellular Lab is intended for owned and enrolled test equipment in controlled RF environments. v0.0.58 establishes the Nougat interface, tabs, architecture, and roadmap foundation while actual private cellular-network transmission remains later implementation work dependent on appropriate hardware and controlled spectrum conditions.
