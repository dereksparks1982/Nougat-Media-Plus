...
# Nougat Media Suite v0.0.27 changed-files installer.
# Same-version repair: post-apply validation uses retained-v26 compatibility tests, not the v26 identity contract.
# Approved scope: Home/Continue Watching, local personalized card wall, higher-quality
# artwork and silent hover previews, persistent resume/Stop UX, now-playing identity,
# rounded windowed video, seek-hover previews, mouse-motion flicker/notch repair,
# and requested Discover/Stream/Debug wording cleanup. P2P expansion remains v0.0.28.

payload_root="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
project_root="$HOME/DKLab/Projects/Nougat Media Suite"
manifest_path="$payload_root/NOUGAT_MEDIA_SUITE_PATCH_MANIFEST_v27.json"
archive_parent="$HOME/DKLab/Archives/ReddMedia Archive"
stamp="$(date +%Y%m%d_%H%M%S)"
rollback_root="$archive_parent/Nougat_Media_Suite_pre_v0_0_27_HOME_RESUME_PLAYER_POLISH_$stamp"
build_root="${TMPDIR:-/tmp}/nougat-media-suite-v0_0_27-$stamp-$$"
expected_head="406c910e606ba58fa8943b1973e45d135ac15eae"
...
modified_paths=(
  ...
  "tools/test_installer_rollback_v27.py"
  ...
  "INSTALL_NOUGAT_MEDIA_SUITE_v0_0_27.sh"
  "NOUGAT_MEDIA_SUITE_PATCH_MANIFEST_v27.json"
)
...
s.close()
if used:
    print('FAIL: localhost port 8096 is in use. Close Nougat Media Suite/Jellyfin before applying v0.0.27.')
    raise SystemExit(1)
...
verify_git_state() {
  ...
  [[ "$head" == "$expected_head" ]] || { printf 'FAIL: expected accepted v0.0.26 HEAD %s, found %s\n' "$expected_head" "${head:-unknown}"; return 1; }
  [[ -z "$dirty" ]] || { printf 'FAIL: accepted v0.0.26 worktree must be clean before v0.0.27:\n%s\n' "$dirty"; return 1; }
  printf 'Git preflight PASS: clean main at accepted v0.0.26 commit %s.\n' "$expected_head"
}
...
if m.get('target_version') != '0.0.27': print('FAIL: manifest target version mismatch'); raise SystemExit(1)
if m.get('accepted_base_commit') != '406c910e606ba58fa8943b1973e45d135ac15eae': print('FAIL: manifest accepted base mismatch'); raise SystemExit(1)
def sha(p):
    h=hashlib.sha256()
    with p.open('rb') as f:
        for b in iter(lambda:f.read(1024*1024),b''): h.update(b)
...
for rel,record in m.get('base_files',{}).items():
    p=project/rel
    if not p.is_file() or p.stat().st_size!=record['bytes'] or sha(p)!=record['sha256']:
        print(f'FAIL: accepted v0.0.26 base differs: {rel}'); raise SystemExit(1)
for rel in m.get('required_absent',[]):
    if (project/rel).exists(): print(f'FAIL: expected v0.0.26-absent path exists: {rel}'); raise SystemExit(1)
print(f"Manifest verified: {len(m.get('payload',{}))} payload files, {len(m.get('base_files',{}))} exact accepted-v0.0.26 base files, {len(m.get('required_absent',[]))} required-absent paths.")
PY
}
...
print('Licensing, Search engine/bridge, and deferred P2P implementation preserved unchanged for v0.0.27.')
PY
}
...
save_rollback_snapshot() {
  ...
  if [[ -e "$project_root/Nougat_Media_Suite_v26" ]]; then cp -a "$project_root/Nougat_Media_Suite_v26" "$rollback_root/project/Nougat_Media_Suite_v26" || return 1; fi
  for src in "$launcher_unversioned" "$launcher_v22" "$launcher_v23" "$launcher_v24" "$launcher_v25" "$launcher_v26" "$launcher_v27" "$canonical_launcher"; do
...
Created: $(date -Is)
Accepted base: v0.0.26 $expected_head
Target candidate: v0.0.27
Package scope: Home/resume/player polish/seek previews/UI cleanup; v0.0.28 P2P expansion deferred
INFO
}
...
apply_payload() {
  local rel
  for rel in "${modified_paths[@]}"; do
    [[ "$rel" == "NOUGAT_MEDIA_SUITE_PATCH_MANIFEST_v27.json" ]] && continue
    mkdir -p "$project_root/$(dirname "$rel")" || return 1
    cp -a "$payload_root/$rel" "$project_root/$rel" || { printf 'FAIL: could not apply %s\n' "$rel"; return 1; }
  done
  cp -a "$payload_root/NOUGAT_MEDIA_SUITE_PATCH_MANIFEST_v27.json" "$project_root/NOUGAT_MEDIA_SUITE_PATCH_MANIFEST_v27.json" || return 1
  chmod +x "$project_root/INSTALL_NOUGAT_MEDIA_SUITE_v0_0_27.sh" \
    "$project_root/tools/test_nougat_media_suite_retained_v26.py" \
    "$project_root/tools/test_nougat_media_suite_v27.py" \
    "$project_root/tools/test_nougat_media_suite_ui_smoke_v27.py" \
    "$project_root/tools/test_installer_rollback_v27.py" || return 1
  applied=1
}

restore_rollback() {
  [[ "$applied" == "1" ]] || return 0
  printf '\nROLLBACK START: restoring exact accepted v0.0.26 touched state\n'
  ...
  rm -f -- "$project_root/Nougat_Media_Suite_v27"
  if [[ -e "$rollback_root/project/Nougat_Media_Suite_v26" ]]; then cp -a "$rollback_root/project/Nougat_Media_Suite_v26" "$project_root/Nougat_Media_Suite_v26"; chmod +x "$project_root/Nougat_Media_Suite_v26" 2>/dev/null || true; fi
  for dest in "$launcher_unversioned" "$launcher_v22" "$launcher_v23" "$launcher_v24" "$launcher_v25" "$launcher_v26" "$launcher_v27" "$canonical_launcher"; do
  ...
  printf 'ROLLBACK PASS: accepted v0.0.26 touched state restored.\n'
}
...
run_source_tests() {
  ...
  python3 "$project_root/tools/test_nougat_media_suite_retained_v26.py" "$project_root" || return 1
  python3 "$project_root/tools/test_nougat_media_suite_v27.py" "$project_root" || return 1
  python3 "$project_root/tools/test_installer_rollback_v27.py" "$project_root" || return 1
}
...
build_stub() {
  ...
  local exe="$build_root/stub/Nougat_Media_Suite_v27"
  [[ "$("$exe" --version)" == "Nougat Media Suite v0.0.27" ]] || { printf 'FAIL: v27 stub version mismatch\n'; return 1; }
  "$exe" --discover-ai-self-test || return 1
  "$exe" --v25-ui-state-self-test || return 1
  python3 "$project_root/tools/test_nougat_media_suite_retained_v26.py" "$project_root" "$exe" || return 1
  python3 "$project_root/tools/test_nougat_media_suite_v27.py" "$project_root" "$exe" || return 1
  python3 "$project_root/tools/test_nougat_media_suite_ui_smoke_v27.py" "$project_root" "$exe" || return 1
}
...
build_full() {
  ...
  local exe="$build_root/full/Nougat_Media_Suite_v27"
  verify_relative_ai_rpath "$exe" || return 1
  [[ "$(env -u LD_LIBRARY_PATH "$exe" --version 2>/dev/null)" == "Nougat Media Suite v0.0.27" ]] || { printf 'FAIL: full native v27 version/runtime check failed\n'; return 1; }
...
install_launchers() {
  ...
  local file
  for file in NougatMediaSuite.desktop NougatMediaSuite_v22.desktop NougatMediaSuite_v23.desktop NougatMediaSuite_v24.desktop NougatMediaSuite_v25.desktop NougatMediaSuite_v26.desktop NougatMediaSuite_v27.desktop com.elderredsoftworks.NougatMediaSuite.desktop; do
...
legacy={'NougatMediaSuite.desktop','NougatMediaSuite_v22.desktop','NougatMediaSuite_v23.desktop','NougatMediaSuite_v24.desktop','NougatMediaSuite_v25.desktop','NougatMediaSuite_v26.desktop','NougatMediaSuite_v27.desktop',canonical}
try:
...
apply_raw_icon() {
  ...
  [[ "$info" == *"file://$master_icon"* ]] || { printf 'FAIL: v27 executable custom-icon readback mismatch\n%s\n' "$info"; return 1; }
  printf 'Raw executable approved N custom-icon metadata verified after final write.\n'
}

install_final_candidate() {
  cp "$build_root/full/Nougat_Media_Suite_v27" "$project_root/Nougat_Media_Suite_v27" || return 1
  chmod +x "$project_root/Nougat_Media_Suite_v27" || return 1
  verify_relative_ai_rpath "$project_root/Nougat_Media_Suite_v27" || return 1
  [[ "$(env -u LD_LIBRARY_PATH "$project_root/Nougat_Media_Suite_v27" --version 2>/dev/null)" == "Nougat Media Suite v0.0.27" ]] || return 1
  env -u LD_LIBRARY_PATH "$project_root/Nougat_Media_Suite_v27" --discover-ai-self-test || return 1
  env -u LD_LIBRARY_PATH "$project_root/Nougat_Media_Suite_v27" --v25-ui-state-self-test || return 1
  python3 "$project_root/tools/test_nougat_media_suite_retained_v26.py" "$project_root" "$project_root/Nougat_Media_Suite_v27" || return 1
  python3 "$project_root/tools/test_nougat_media_suite_v27.py" "$project_root" "$project_root/Nougat_Media_Suite_v27" || return 1
  python3 "$project_root/tools/test_nougat_media_suite_ui_smoke_v27.py" "$project_root" "$project_root/Nougat_Media_Suite_v27" || return 1
  install_icon_aliases || return 1
  install_launchers || return 1
  refresh_nougat_favorite || return 1
  apply_raw_icon "$project_root/Nougat_Media_Suite_v27" || return 1
  rm -f -- "$project_root/Nougat_Media_Suite_v26" || return 1
  command -v nautilus >/dev/null 2>&1 && nautilus -q >/dev/null 2>&1 || true
  return 0
}

main() {
  phase_start "accepted v0.0.26 base and v0.0.27 prerequisites"
  [[ -d "$project_root" ]] || { printf 'FAIL: project not found: %s\n' "$project_root"; printf '\nFINAL FAIL: v0.0.27 not installed. Terminal remains open.\n'; return 1; }
  local cmd
  ...
  [[ -x "$project_root/Nougat_Media_Suite_v26" ]] || { printf 'FAIL: accepted v0.0.26 root executable missing/not executable.\n'; return 1; }
  [[ "$(env -u LD_LIBRARY_PATH "$project_root/Nougat_Media_Suite_v26" --version 2>/dev/null)" == "Nougat Media Suite v0.0.26" ]] || { printf 'FAIL: accepted v0.0.26 executable version/runtime check failed.\n'; return 1; }
  ...
  verify_protected_state || { printf '\nFINAL FAIL: protected accepted-v0.0.26 state mismatch. Terminal remains open.\n'; return 1; }
  verify_manifest || { printf '\nFINAL FAIL: v0.0.27 package/base verification failed. Terminal remains open.\n'; return 1; }
  phase_pass "accepted v0.0.26 base and v0.0.27 prerequisites"

  phase_start "save exact accepted v0.0.26 rollback snapshot"
  save_rollback_snapshot || { printf '\nFINAL FAIL: rollback snapshot failed before changes. Terminal remains open.\n'; return 1; }
  phase_pass "save exact accepted v0.0.26 rollback snapshot"

  phase_start "apply approved Nougat Media Suite v0.0.27 changed files"
  apply_payload || { restore_rollback; printf '\nFINAL FAIL: v0.0.27 payload application failed. Terminal remains open.\n'; return 1; }
  phase_pass "apply approved Nougat Media Suite v0.0.27 changed files"

  phase_start "license, Search/P2P preservation, retained v0.0.26 behavior, and v0.0.27 regression tests"
  run_source_tests || { restore_rollback; printf '\nFINAL FAIL: source/regression validation failed; accepted v0.0.26 touched state restored. Terminal remains open.\n'; return 1; }
  phase_pass "license, Search/P2P preservation, retained v0.0.26 behavior, and v0.0.27 regression tests"

  phase_start "warnings-as-errors stub build and X11 v0.0.27 identity/UI smoke"
  mkdir -p "$build_root" && build_stub || { restore_rollback; printf '\nFINAL FAIL: deterministic build/UI validation failed; accepted v0.0.26 touched state restored. Terminal remains open.\n'; return 1; }
  phase_pass "warnings-as-errors stub build and X11 v0.0.27 identity/UI smoke"

  phase_start "full native Nougat Media Suite v0.0.27 rebuild"
  build_full || { restore_rollback; printf '\nFINAL FAIL: full native v0.0.27 build failed; accepted v0.0.26 touched state restored. Terminal remains open.\n'; return 1; }
  phase_pass "full native Nougat Media Suite v0.0.27 rebuild"

  phase_start "install and verify final v27 executable, launchers, and N identity"
  install_final_candidate || { restore_rollback; printf '\nFINAL FAIL: final v0.0.27 installation failed; accepted v0.0.26 touched state restored. Terminal remains open.\n'; return 1; }
  phase_pass "install and verify final v27 executable, launchers, and N identity"

  rm -rf -- "$build_root"
  printf '\nFINAL PASS: Nougat Media Suite v0.0.27 Home, Resume History, Player Polish, and Seek Previews installed and validated.\n'
  printf 'Executable: %s\n' "$project_root/Nougat_Media_Suite_v27"
  printf 'Rollback snapshot: %s\n' "$rollback_root"
  printf 'LICENSE STATUS: protected PolyForm Noncommercial licensing files preserved unchanged.\n'
  printf 'SEARCH/P2P STATUS: Search engine/bridge and existing P2P implementation preserved unchanged; focused P2P expansion remains roadmap v0.0.28.\n'
  printf 'LATER VISUAL PROPOSAL: logged only, not applied in v0.0.27: purple Home background; cocoa/chocolate/caramel Video Player; green Library; red Discover; cream Search; provider-reactive Stream; charcoal Debug; darker blended windowed-video surround.\n'
  printf 'OWNER CHECK REQUIRED: Home/default tab; Continue Watching wheel/progress; LOCAL genre/recommendation wall; artwork/hover preview; persistent resume; Stop screen; title strip/fullscreen overlay; rounded windowed video; seek preview; flicker repair; selected-notch layering; requested wording cleanup; retained v0.0.26 behavior.\n'
  printf 'CANDIDATE STATUS: v0.0.27 remains unaccepted until owner real-machine visual/use approval.\n'
  printf 'Launch: cd "%s" && ./Nougat_Media_Suite_v27\n' "$project_root"
}
