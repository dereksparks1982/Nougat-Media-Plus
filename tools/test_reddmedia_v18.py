#!/usr/bin/env python3
"""Deterministic v0.0.18 metadata, provider, diagnostic, and preference validation."""
...
for marker in (
    ...
    "library_grid_metrics()",
    "grid.visibleItems",
    "draw_debug_screen(buffer)",
):
    require(marker in main_source, f"missing v0.0.18 UI behavior marker: {marker}")
...
with tempfile.TemporaryDirectory(prefix="reddmedia-v18-tests-") as temporary:
    ...
    try:
        ...
        raise SystemExit(f"v0.0.18 metadata test requires free localhost port 8096: {exc}") from exc
...
print("ReddMedia v0.0.18 deterministic validation PASS")
