...
need('VERSION 0.0.36' in cm and 'Nougat_Play_Portal_v36' in cm and 'Nougat_Play_Portal_v35' not in cm,
     'v0.0.36 CMake/target identity missing')

# Protected README description must remain verbatim directly under the current title.
protected = ('# Nougat Play Portal v0.0.36\n\n'
             'Nougat Play Portal is the new official identity of the Linux media application previously released as ReddMedia through accepted v0.0.20. '
...
need('const int badgeY = (kTopBarH - nougat_play_portal_icon::kTopBar14Size) / 2;' in main and
...
for rel in ('NougatPlayPortal.desktop','NougatPlayPortal_v36.desktop','com.elderredsoftworks.NougatPlayPortal.desktop'):
    text = (root / rel).read_text(errors='replace')
    need('Nougat_Play_Portal_v36' in text, 'v36 launcher identity missing: ' + rel)
...
if exe:
    ...
    need(out == 'Nougat Play Portal v0.0.36', 'v36 version mismatch: ' + repr(out))
