...
binary=pathlib.Path(sys.argv[2]).resolve() if len(sys.argv)>2 else root/'Nougat_Media_Suite_v27'
...
with tempfile.TemporaryDirectory(prefix='nms-v27-ui.') as raw:
    ...
    exe=app/'Nougat_Media_Suite_v27'; shutil.copy2(binary,exe); exe.chmod(0o755)
    script=f'''cd "{app}"
...
print('nougat-media-suite-v27-ui-window=pass canonical-app-id=pass wm-class=pass dock-desktop-hint=pass net-wm-icon=pass default-home-window=pass')
