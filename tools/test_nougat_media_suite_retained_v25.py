...
BINARY = pathlib.Path(sys.argv[2]).resolve() if len(sys.argv) > 2 else ROOT / "Nougat_Media_Suite_v26"
...
need("VERSION 0.0.26" in cmake, "CMake version is not v0.0.25")
need("add_executable(Nougat_Media_Suite_v26" in cmake, "v26 executable target missing")
need("Nougat Media Suite v0.0.26" in main, "CLI v26 version marker missing")
need('const std::string versionLabel = "v0.0.26"' in main, "in-app v26 marker missing")
...
need(BINARY.is_file(), "v26 retained-v25 test binary missing")
...
with tempfile.TemporaryDirectory(prefix="nougat-v26-retained-v25-resolver-") as raw:
    ...
    try:
        ...
        raise SystemExit(f"FAIL: v26 retained-v25 resolver test requires free localhost port 8096: {exc}") from exc
...
print("retained-v25-contract=pass stream-provider-theme=pass stream-provider-notch=pass stream-redundant-header-removed=pass stream-provider-panel-removed=pass discover-dual-notch=pass discover-local-play=pass history-resume=pass search-preserved=pass license-preserved=pass")
