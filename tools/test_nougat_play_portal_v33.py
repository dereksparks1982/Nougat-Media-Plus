...
exe=Path(sys.argv[2]).resolve() if len(sys.argv)>2 else root/'Nougat_Play_Portal_v33'
def need(ok,msg):
...
need('VERSION 0.0.33' in cm and 'Nougat_Play_Portal_v33' in cm,'v33 build identity missing')
need('printf("Nougat Play Portal v0.0.33\\n")' in main and 'const std::string versionLabel = "v0.0.33"' in main,'v33 runtime/UI identity missing')
...
if exe.is_file():
    need(subprocess.check_output([str(exe),'--version'],text=True).strip()=='Nougat Play Portal v0.0.33','v33 executable version mismatch')
