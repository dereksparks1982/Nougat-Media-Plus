...
if exe is not None:
    need(subprocess.check_output([str(exe),'--version'],text=True).strip()=='Nougat Play Portal v0.0.30','v30 executable version mismatch')
    subprocess.check_call([str(exe),'--v28-ui-state-self-test'])
    subprocess.check_call([str(exe),'--v29-tv-reliability-self-test'])
print('retained-v29=pass tv-v29=pass home-resume=pass palette=pass library-posters=pass search-cleanup=pass seek-preview=pass back-forward=pass volume-200=pass diagnostics=pass search-p2p-license=pass')
