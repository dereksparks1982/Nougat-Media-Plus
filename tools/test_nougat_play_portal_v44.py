...
def main() -> None:
    ...
    print(f"=== NOUGAT PLAY PORTAL v0.0.44 REGRESSION ===\nProject: {root}")

    cmake = read(root, "CMakeLists.txt")
    require(cmake, "project(NougatPlayPortal VERSION 0.0.44 LANGUAGES CXX)", "CMake version")
    require(cmake, "add_executable(Nougat_Play_Portal_v44", "CMake target")
    require(cmake, "target_compile_options(Nougat_Play_Portal_v44 PRIVATE -Wall -Wextra -Werror)", "warnings-as-errors")
    forbid(cmake, "Nougat_Play_Portal_v43", "CMake stale target")
    ...
    for rel in ["NougatPlayPortal.desktop", "com.elderredsoftworks.NougatPlayPortal.desktop"]:
        ...
        require(desktop, "Nougat_Play_Portal_v44", rel)
        require(desktop, "Icon=nougat-play-portal-concept-sheet-v24", rel)
        forbid(desktop, "Nougat_Play_Portal_v43", rel)
    ...
    if not readme.startswith("# Nougat Play Portal\n\n"):
        fail("README title must remain exactly '# Nougat Play Portal' with no version in title")
    ...
    icon = root / "assets/icons/nougat-play-portal-concept-sheet-v24.png"
    x11 = root / "src/nougat_play_portal_icon_data.hpp"
    ...
    if len(sys.argv) > 2:
        ...
        if version.returncode != 0 or (version.stdout + version.stderr).strip() != "Nougat Play Portal v0.0.44":
