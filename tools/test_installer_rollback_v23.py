...
INSTALLER = ROOT / 'INSTALL_NOUGAT_PLAY_PORTAL_v0_0_23.sh'
...
assert {
    ...
    'DEPENDENCIES.md', 'NougatPlayPortal.desktop', 'README.md', 'ROADMAP.md',
    'assets/icons/nougat-play-portal.png', 'docs/NOUGAT_PLAY_PORTAL_BRAND_PALETTE.md',
    'src/main.cpp', 'src/nougat_play_portal_icon_data.hpp', 'src/ytdlp_stream_server.cpp',
}.issubset(modified)
assert {
    'assets/icons/nougat-play-portal-14.png', 'NougatPlayPortal_v23.desktop',
    'INSTALL_NOUGAT_PLAY_PORTAL_v0_0_23.sh',
    'NOUGAT_PLAY_PORTAL_PATCH_MANIFEST_v23.json',
    'docs/builds/NOUGAT_PLAY_PORTAL_v0_0_23_EXACT_CONCEPT_UI_STREAM_REPAIR_HANDSHAKE.md',
    'docs/builds/NOUGAT_PLAY_PORTAL_v0_0_23_EXACT_CONCEPT_UI_STREAM_REPAIR_VALIDATION.md',
    'tools/test_installer_rollback_v23.py', 'tools/test_nougat_play_portal_ui_smoke_v23.py',
    'tools/test_nougat_play_portal_v23.py',
}.issubset(added)
assert deleted == ['NougatPlayPortal_v22.desktop']
...
assert 'Nougat_Play_Portal_v22' in source and 'Nougat_Play_Portal_v23' in source
...
assert 'env -u LD_LIBRARY_PATH "$project_root/Nougat_Play_Portal_v23" --discover-ai-self-test' in source
...
assert 'rm -f -- "$project_root/Nougat_Play_Portal_v22"' in source
assert 'rm -f -- "$project_root/NougatPlayPortal_v22.desktop"' in source
assert 'FINAL PASS: Nougat Play Portal v0.0.23 exact-concept UI and Stream repair candidate installed and validated.' in source
...
with tempfile.TemporaryDirectory(prefix='nms-v23-rollback.') as raw:
    ...
    old = project / 'Nougat_Play_Portal_v22'
    old.write_text('current-v22\n', encoding='utf-8')
    shutil.copy2(old, snap / 'Nougat_Play_Portal_v22')
    ...
    for rel in deleted:
        ...
        if p.exists():
            p.unlink()
    old.unlink()
    (project / 'Nougat_Play_Portal_v23').write_text('candidate-v23\n', encoding='utf-8')
    ...
    for rel in added:
        ...
        if p.is_dir():
            ...
            p.unlink()
    new = project / 'Nougat_Play_Portal_v23'
    if new.exists():
        new.unlink()
    for rel in modified + deleted:
        p = project / rel
    ...
    shutil.copy2(snap / 'Nougat_Play_Portal_v22', project / 'Nougat_Play_Portal_v22')

    for rel in modified + deleted:
        assert (project / rel).read_text(encoding='utf-8') == f'base:{rel}\n'
    assert all(not (project / rel).exists() for rel in added)
    assert not (project / 'Nougat_Play_Portal_v23').exists()
    assert (project / 'Nougat_Play_Portal_v22').read_text(encoding='utf-8') == 'current-v22\n'
