...
def compile_changed_cpp(root: Path) -> None:
    compiler = shutil.which("g++")
    if not compiler:
        fail("g++ is required for v45 module compile validation")
    files = [
    ...
    with tempfile.TemporaryDirectory(prefix="nougat-v45-obj-") as td:
        for rel in files:
    ...
    ok("all new/changed v45 C++ security modules compile with warnings-as-errors")


def privacy_canary_test(root: Path) -> None:
    worker = root / "components/nougat/nougat_search_worker.py"
    with tempfile.TemporaryDirectory(prefix="nougat-v45-privacy-") as td:
        env = os.environ.copy()
        env["NOUGAT_HOME"] = td
...
def main() -> None:
    ...
    require(main_cpp, "reddmedia::SecureSearchController::search(nougat, query, raw, include_peers, 100, offset)",
    ...
    ok("main.cpp contains controller wiring only, not privacy implementation")
    ...
    ok("Search and crawler are separate processes and Search query is not argv")
    ...
    ok("local Search worker has no direct network client path")
    ...
    ok("silent live-discovery and plaintext peer Search paths are disabled")
    ...
    for state in ["ROBOTS RESTRICTED", "BOT POLICY BLOCKED", "RATE LIMITED", "AUTH REQUIRED",
    ...
    ok("Crawler Access states and no-auto-payment rule are present")
    ...
    ok("Privacy Receipt v1 has no query field")
    ...
    ok("Privacy Law invariants are explicit")
    ...
    ok("Privacy Broker protocol is versioned and remote query transport remains fail-closed")
    ...
    if exe.is_file() and os.access(exe, os.X_OK):
        ...
        if selftest.returncode != 0:
            fail(f"retained v44 native release self-test failed: {(selftest.stdout + selftest.stderr)[-3000:]}")
        ok("native v45 executable identity and retained v44 release self-test")
    else:
        print("NOTE: root v45 executable not present yet; native runtime gate will run after build/install.")

    print("=== v0.0.45 SECURE SEARCH VALIDATION PASS ===")
