...
desktop=(root/'NougatPlayPortal.desktop').read_text(encoding='utf-8')
legacy_v22_desktop=(root/'NougatPlayPortal_v22.desktop').read_text(encoding='utf-8')
legacy_v23_desktop=(root/'NougatPlayPortal_v23.desktop').read_text(encoding='utf-8')
versioned_desktop=(root/'NougatPlayPortal_v24.desktop').read_text(encoding='utf-8')
canonical_desktop=(root/'com.elderredsoftworks.NougatPlayPortal.desktop').read_text(encoding='utf-8')
...
need('add_executable(Nougat_Play_Portal_v24' in cmake, 'v24 executable target missing')
need('Nougat_Play_Portal_v23' not in cmake, 'v23 target leaked into CMake')
need('Nougat Play Portal v0.0.24' in main, 'CLI v0.0.24 marker missing')
need('const std::string versionLabel = "v0.0.24"' in main, 'in-app v0.0.24 marker missing')
need('Nougat_Play_Portal_v24' in desktop, 'desktop launcher does not target v24')
...
need('text(target, 28, 17, "NOUGAT PLAY PORTAL"' in bar, 'suite title is not immediately beside left N')
...
need('_GTK_APPLICATION_ID' in identity and 'const char* appId = "com.elderredsoftworks.NougatPlayPortal"' in identity,
     'canonical GNOME application-ID hint missing')
need('_BAMF_DESKTOP_FILE' in identity and 'com.elderredsoftworks.NougatPlayPortal.desktop' in identity,
     'Ubuntu dock desktop-file identity hint missing')
need('StartupWMClass=NougatPlayPortal' in desktop, 'desktop StartupWMClass mismatch')
need('Icon=nougat-play-portal-concept-sheet-v24' in desktop, 'desktop does not use fresh exact-N icon-theme key')
need('X-GNOME-Application-ID=com.elderredsoftworks.NougatPlayPortal' in desktop, 'desktop canonical GNOME application ID missing')
for name,ds in [('unversioned',desktop),('v22',legacy_v22_desktop),('v23',legacy_v23_desktop),('v24',versioned_desktop),('canonical',canonical_desktop)]:
    need('Icon=nougat-play-portal-concept-sheet-v24' in ds, f'{name} launcher still uses old icon key')
    need('StartupWMClass=NougatPlayPortal' in ds, f'{name} launcher StartupWMClass mismatch')
    need('X-GNOME-Application-ID=com.elderredsoftworks.NougatPlayPortal' in ds, f'{name} launcher GNOME app ID mismatch')
icon=root/'assets/icons/nougat-play-portal.png'
...
for asset in (root/'assets/icons').glob('nougat-play-portal*.png'):
...
need(hashlib.sha256((root/'src/nougat_play_portal_icon_data.hpp').read_bytes()).hexdigest()=='0fa96e1b4d79369732eedd4d8da472a8e5f72f99b44ace3885fa724834907809',
...
icon_header=(root/'src/nougat_play_portal_icon_data.hpp').read_text(encoding='utf-8')
