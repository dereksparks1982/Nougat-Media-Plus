#!/usr/bin/env python3
"""Bounded Xvfb startup proof for ReddMedia v0.0.19."""
from __future__ import annotations
import os, pathlib, shutil, subprocess, sys, tempfile, textwrap
ROOT = pathlib.Path(sys.argv[1] if len(sys.argv) > 1 else '.').resolve()
BINARY = pathlib.Path(sys.argv[2]).resolve() if len(sys.argv) > 2 else ROOT / 'ReddMedia_v19'
...
with tempfile.TemporaryDirectory(prefix='reddmedia-v19-ui-smoke.') as raw:
    work = pathlib.Path(raw)
    app = work/'app'; app.mkdir()
    home = work/'home'; home.mkdir()
    shutil.copy2(BINARY, app/'ReddMedia_v19')
    ...
    script = textwrap.dedent('''
        ...
        kill "$pid" 2>/dev/null || true
        wait "$pid" 2>/dev/null || true
        echo 'reddmedia-ui-window=pass startup-alive=pass'
    ''')
