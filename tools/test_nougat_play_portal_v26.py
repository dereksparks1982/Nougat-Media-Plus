...
need('VERSION 0.0.26' in cm and 'Nougat_Play_Portal_v26' in cm,'CMake v26 identity missing')
need('printf("Nougat Play Portal v0.0.26\\n")' in main,'runtime version missing')
...
master=root/'assets/icons/nougat-play-portal.png'; w,h,rows=png_rgba(master); need((w,h)==(512,512),'N master size wrong')
...
for size,hx in iconhashes.items(): need(sha(f'assets/icons/nougat-play-portal-{size}.png')==hx,f'clean N {size} hash mismatch')
need(sha('src/nougat_play_portal_icon_data.hpp')=='c626664598d57a3756a62a425875fd48567ae4eaa8c9b5a385ccf4630a0b22cb','embedded clean N data mismatch')
if exe is not None:
    import subprocess
    need(subprocess.check_output([str(exe),'--version'],text=True).strip()=='Nougat Play Portal v0.0.26','executable version mismatch')
