...
BINARY=pathlib.Path(sys.argv[2]).resolve() if len(sys.argv)>2 else ROOT/'Nougat_Play_Portal_v22'
...
with tempfile.TemporaryDirectory(prefix='nms-v22-ui.') as raw:
    w=pathlib.Path(raw); app=w/'app'; home=w/'home'; app.mkdir(); home.mkdir()
    shutil.copy2(BINARY,app/'Nougat_Play_Portal_v22')
    ...
    ./Nougat_Play_Portal_v22 &
    pid=$!
    sleep 2
    if ! kill -0 "$pid" 2>/dev/null; then echo early-exit; wait "$pid"; return 20; fi
    if ! xwininfo -root -tree 2>/dev/null | grep -F 'Nougat Play Portal' >/dev/null; then echo window-not-found; kill "$pid" 2>/dev/null || true; wait "$pid" 2>/dev/null || true; return 21; fi
    kill "$pid" 2>/dev/null || true
    wait "$pid" 2>/dev/null || true
    echo nougat-play-portal-v22-ui-window=pass
    """)
