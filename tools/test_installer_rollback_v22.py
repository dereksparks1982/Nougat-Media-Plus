...
INSTALLER=ROOT/'INSTALL_NOUGAT_PLAY_PORTAL_v0_0_22.sh'
...
assert {'LICENSE','README.md','THIRD_PARTY_NOTICES.md','COMPANY_BIBLE.md','CMakeLists.txt','src/main.cpp','NougatPlayPortal.desktop'}.issubset(modified)
assert {'COPYRIGHT.md','CONTRIBUTING.md','docs/LICENSING_POLICY.md','.github/PULL_REQUEST_TEMPLATE.md','NougatPlayPortal_v22.desktop','tools/test_license_protection_v22.py','tools/test_nougat_play_portal_v22.py','tools/test_nougat_play_portal_retained_v22.py','tools/test_nougat_play_portal_ui_smoke_v22.py'}.issubset(added)
assert {'INSTALL_NOUGAT_PLAY_PORTAL_v0_0_21.sh','NOUGAT_PLAY_PORTAL_PATCH_MANIFEST_v21.json','NougatPlayPortal_v21.desktop','tools/test_nougat_play_portal_v21.py'}.issubset(deleted)
assert 'project_root="$HOME/DKLab/Projects/Nougat Play Portal"' in source
assert 'expected_base_commit="b89b0fc187bebc05f3390a76c76cbdc980713600"' in source
assert 'expected_base_tag="v0.0.21"' in source
assert 'Nougat_Play_Portal_v21' in source and 'Nougat_Play_Portal_v22' in source
...
assert 'env -u LD_LIBRARY_PATH "$project_root/Nougat_Play_Portal_v22" --discover-ai-self-test' in source
assert 'components/ai/runtime/lib/libllama.so.0' in source
assert 'Nougat Play Portal v0.0.22' in source
assert 'FINAL PASS: Nougat Play Portal v0.0.22 license-protection/runtime-path-repair candidate installed and validated.' in source
...
assert '"$project_root/Nougat_Play_Portal_v22" --discover-ai-self-test' in source
assert 'gio set -t string "$exe" metadata::custom-icon "file://$icon"' in source
assert 'rm -f -- "$project_root/Nougat_Play_Portal_v21"' in source
...
with tempfile.TemporaryDirectory(prefix='nms-v22-rollback.') as raw:
    ...
    old=project/'Nougat_Play_Portal_v21'; old.write_text('accepted-v21\n'); shutil.copy2(old,snap/'Nougat_Play_Portal_v21')
    ...
    old.unlink(); (project/'Nougat_Play_Portal_v22').write_text('candidate-v22\n')
    for rel in added:
        p=project/rel
        if p.is_dir(): shutil.rmtree(p)
        elif p.exists(): p.unlink()
    new=project/'Nougat_Play_Portal_v22'
    if new.exists(): new.unlink()
    for rel in modified+deleted:
        p=project/rel; p.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(snap/rel,p)
    shutil.copy2(snap/'Nougat_Play_Portal_v21',project/'Nougat_Play_Portal_v21')
    for rel in modified+deleted: assert (project/rel).read_text()==f'base:{rel}\n'
    assert all(not (project/rel).exists() for rel in added)
    assert not (project/'Nougat_Play_Portal_v22').exists()
    assert (project/'Nougat_Play_Portal_v21').read_text()=='accepted-v21\n'
