...
def main() -> int:
    try:
        ...
        desktops = [
            (ROOT / "NougatPlayPortal.desktop").read_text(encoding="utf-8"),
            (ROOT / "com.elderredsoftworks.NougatPlayPortal.desktop").read_text(encoding="utf-8"),
        ]

        contains_all(cmake, [
            "VERSION 0.0.49", "Nougat_Play_Portal_v49", "-Wall -Wextra -Werror"
        ], "v49 CMake")
        need("Nougat_Play_Portal_v48" not in cmake, "CMake still targets v48")
        need(all("Nougat_Play_Portal_v49" in desktop for desktop in desktops),
