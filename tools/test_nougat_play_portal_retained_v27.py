...
# v27 Home/resume/player/seek features survive v28 visual/state work.
for token in [
...
]: need(token in main,'retained v27 contract missing: '+token)
...
need('"Direct Play URL"' not in main and '"DIAGNOSTIC CENTER"' not in main,'redundant v27 heading returned')
need('Paste URL Then Press Direct Watch / Rumble / RuTube / VK / OK' in main,'Stream placeholder regressed')

# Diagnostics remain functional.  Do not claim the owner-observed TV Up Next runtime
# path is healthy here; that focused reliability repair is explicitly v0.0.29.
for token in ['Nougat Play Portal Diagnostic Report','report_json','write_support_bundle','[REDACTED SENSITIVE LINE]']:
...
if exe is not None:
    need(subprocess.check_output([str(exe),'--version'],text=True).strip()=='Nougat Play Portal v0.0.28','v28 executable version mismatch')
print('retained-v27=pass home-resume=pass seek-preview=pass back-forward=pass volume-200=pass diagnostics=pass search-p2p-license=pass tv-up-next-runtime=deferred-v29')
