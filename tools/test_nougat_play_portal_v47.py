...
need(readme.startswith("# Nougat Play Portal\n\n**Nougat Play Portal** is a native Linux media center"),
...
need(sha("assets/icons/nougat-play-portal-concept-sheet-v24.png") ==
...
for desktop in ("NougatPlayPortal.desktop", "com.elderredsoftworks.NougatPlayPortal.desktop"):
    d = read(desktop)
    need("Nougat_Play_Portal_v47" in d, desktop + " does not target v47")
    need("Icon=nougat-play-portal-concept-sheet-v24" in d,
         desktop + " lost approved N icon key")
    need("StartupWMClass=NougatPlayPortal" in d, desktop + " lost StartupWMClass")
    need("X-GNOME-Application-ID=com.elderredsoftworks.NougatPlayPortal" in d,
         desktop + " lost GNOME application ID")
    need("X-GNOME-WMClass=NougatPlayPortal" in d, desktop + " lost GNOME WM class")
...
if exe is not None:
    ...
    need(r.returncode == 0 and r.stdout.strip() == "Nougat Play Portal v0.0.47",
...
print("Nougat Play Portal v0.0.47 contract PASS")
