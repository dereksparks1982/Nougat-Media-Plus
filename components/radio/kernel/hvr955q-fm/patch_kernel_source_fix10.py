...
def patch_video(path):
    ...
    if MARK + "_RADIO_OPEN" not in text:
        ...
        replacement = (
            ...
            + indent + '\tcx231xx_set_audio_input(dev, AUDIO_INPUT_TUNER_FM);\n'
            + indent + '}'
        )
...
def patch_avcore(path):
    ...
    s, e = function_span(text, "int cx231xx_set_audio_input(")
    func = text[s:e]

    # Ubuntu's exact source may place default: beside or away from the TV case.
    # Do not depend on that layout. First remove only the abandoned FM stub,
    # then add FM as an additional label on the established tuner-TV SIF path.
    fm_case = re.compile(
    ...
    abandoned = [
        m for m in fm_matches
        if ("setupFM()" in m.group("body")
            or "use SIF for FM radio" in m.group("body"))
    ]
    if len(abandoned) != 1:
        die(
            "abandoned FM audio stub: expected one unfinished FM case, "
            f"found {len(abandoned)}"
        )
    ...
    if len(tv_matches) != 1:
        die(
            "cx231xx tuner TV audio path: expected one TV case, "
    ...
    insertion = (
        ...
        + indent + "\t/* " + MARK
        + "_FM_AUDIO: share the established tuner SIF path. */\n"
    )
    func = func[:m.start()] + insertion + func[m.start():]
...
def main():
    if len(sys.argv) != 2:
        die("usage: patch_kernel_source_fix10.py <exact-source-root>")
