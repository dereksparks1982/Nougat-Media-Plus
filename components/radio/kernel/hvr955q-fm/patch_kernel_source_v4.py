...
OLD_MARK = "NOUGAT_V67_HVR955Q_FM_TWO_MODULE"
MARK = "NOUGAT_V67_HVR955Q_FM_MATCHED_STACK"
...
def patch_cards(path):
    text = read(path)
    if OLD_MARK + "_RADIO_NODE" in text or MARK + "_RADIO_NODE" in text:
        return
...
def patch_video(path):
    ...
    if OLD_MARK + "_RADIO_CAPS" not in text and MARK + "_RADIO_CAPS" not in text:
...
\treturn 0;
    ...
    if OLD_MARK + ": preserve the stock cx231xx-dvb callback ABI." not in text and MARK + "_FREQ_DISPATCH" not in text:
...
\t\t/* __MARK___FREQ_DISPATCH: zero norm marks FM without changing public ABI. */
...
\t\tbreak;'''.replace("__MARK__", MARK)
    ...
    if OLD_MARK + ": HVR-9xx FM IF" not in text and MARK + "_FM_IF" not in text:
...
\t\tif (f->type == V4L2_TUNER_RADIO)
\t\t\tif_frequency = 6600000;  /* __MARK___FM_IF: HVR-9xx FM */
...
\t\t\tif_frequency = 5400000;  /*5.4MHz\t*/'''.replace("__MARK__", MARK)
    ...
    if OLD_MARK + "_RADIO_OPEN" not in text and MARK + "_RADIO_OPEN" not in text:
...
def patch_dvb(path):
    ...
    # Match the real kernel source regardless of spaces/tabs. FIX7 accidentally
    # searched for the two literal characters "\\t", so it could never match
    # the actual tab in cx231xx-dvb.c.
    pattern = re.compile(
    ...
    if len(matches) != 1:
        die(
            "cx231xx-dvb FM mode propagation: expected one real source line, "
            f"found {len(matches)}"
        )
    ...
    replacement = (
        indent
        + "params.mode = dev->norm ? 0 : V4L2_TUNER_RADIO;"
        + "\t/* "
        + MARK
        + "_DVB_RADIO_MODE */"
    )
...
def patch_si2157(path):
    text = read(path)
    if OLD_MARK + "_SI2157_FM" in text or MARK + "_SI2157_FM" in text:
        return
...
'''
    func = replace_once(func, active_anchor, active_new, "Si2157 FM mode")
    ...
    func = (
        func[:m.start()]
        + "\t/* " + MARK + "_SI2157_FM: HVR FM enabled. */\n"
        + func[m.end():]
    )
...
def main():
    if len(sys.argv) != 2:
        die("usage: patch_kernel_source_v4.py <exact-source-root>")
    ...
    print("PASS: HVR-955Q FM matched-stack patch applied")
    print("PASS: public cx231xx header ABI untouched")
    print("PASS: cx231xx-dvb sends explicit RADIO mode to Si2157")
    print("PASS: 6.6 MHz FM IF and V4L2 radio node enabled")
