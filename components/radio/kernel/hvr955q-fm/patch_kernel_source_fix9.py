...
MARK = "NOUGAT_V67_HVR955Q_FM_END_TO_END_FIX9"
...
def patch_avcore(path):
    ...
    tv_case = "\tcase AUDIO_INPUT_TUNER_TV:\n\tdefault:"
    if tv_case not in func:
        die("cx231xx tuner TV audio path anchor not found")
    func = func.replace(
        tv_case,
        "\tcase AUDIO_INPUT_TUNER_FM:\n"
        "\t\t/* " + MARK + "_FM_AUDIO: use the established tuner SIF path. */\n"
    ...
    stub = re.compile(
        r'(?ms)^\tcase AUDIO_INPUT_TUNER_FM:\n'
        r'\t\t/\*\s*use SIF for FM radio\n'
        r'.*?'
        r'^\t\tbreak;\n'
    )
    matches = list(stub.finditer(func))
    if len(matches) != 1:
        die(f"abandoned FM audio stub: expected one anchor, found {len(matches)}")
    func = func[:matches[0].start()] + func[matches[0].end():]
    path.write_text(text[:s] + func + text[e:])
...
def main():
    if len(sys.argv) != 2:
        die("usage: patch_kernel_source_fix9.py <exact-source-root>")
